...
RegisterNetEvent("illenium-appearance:client:reloadSkin", function(bypassChecks)
    ...
    local function isNearStore()
        for _, store in ipairs(Config.Stores) do
            if store.usePoly then
                if IsPointInPolygon(playerCoords, store.points) then
                    return true
                end
            else
                local storeCenter = store.coords
                local storeSize = store.size
                local dist = Vdist(playerCoords.x, playerCoords.y, playerCoords.z, storeCenter.x, storeCenter.y, storeCenter.z)
                if dist < storeSize.x then
                    return true
                end
            end
        end
    ...
    if not bypassChecks and InCooldown() or Framework.CheckPlayerMeta() or cache.vehicle or IsPedFalling(playerPed) then
        lib.notify({
            title = _L("commands.reloadskin.failure.title"),
            description = _L("commands.reloadskin.failure.description"),
            type = "error",
