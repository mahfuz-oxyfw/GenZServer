...
RegisterNetEvent('qb-bankrobbery:server:TrollyReward', function(type, bank, index)
    ...
    if #(coords - Config.Banks[bank].trolly[index].coords.xyz) > 10 then
    ...
    elseif type == 'paleto' then
        local receiveAmount = math.random(5, 10)
        Player.Functions.AddItem('markedbills', receiveAmount, false, info)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add', receiveAmount)
        TriggerClientEvent('QBCore:Notify', src, 'You got ' .. receiveAmount .. ' bags of inked money...')
    ...
    elseif type == 'pacific' then
        local receiveAmount = math.random(10, 15)
        Player.Functions.AddItem('markedbills', receiveAmount, false, info)
        TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['markedbills'], 'add', receiveAmount)
        TriggerClientEvent('QBCore:Notify', src, 'You got ' .. receiveAmount .. ' bags of inked money...')
...
RegisterNetEvent('qb-bankrobbery:server:LockerReward', function(type, bank, index)
    ...
    if type == 'fleeca' then
        ...
        elseif tier == 4 then
            -- Add laptop_blue as a 100% reward
            Player.Functions.AddItem('laptop_blue', 1)
            TriggerEvent('qb-log:server:CreateLog', 'bankrobbery', 'Bankrobbery log: '..type, 'green', '**'.. Player.PlayerData.name .. '** (citizenid: *'..Player.PlayerData.citizenid..'* | id: *'..src..'*) received: **laptop_blue**')
            TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['laptop_blue'], 'add')
            if debug then
                print('^3[qb-bankrobbery] ^5'..Player.PlayerData.name..'(citizenid: '..Player.PlayerData.citizenid..' | id: '..src..') received laptop_blue^7')
            end        
        elseif tier == 4 then
            --blue usb
            Player.Functions.AddItem('usb_blue', 1)
