...
local function healOxy()
    ...
    local healthToAdd = maxHealth * 0.25 -- 25% of max health
    local healthPerTick = healthToAdd / 9 -- Spread over 9 seconds
    local count = 9

    while count > 0 do
        Wait(1000)
        count -= 1
        SetEntityHealth(player, GetEntityHealth(player) + healthPerTick)
    end
...
RegisterNetEvent('consumables:client:oxy', function()
    QBCore.Functions.Progressbar('use_oxy', Lang:t('consumables.healing_progress'), 2000, false, true, {
        ...
        healOxy()
        TriggerServerEvent('hud:server:RelieveStress', 50) -- Relieve exactly 50% stress
    end, function() -- Cancel
        StopAnimTask(PlayerPedId(), 'mp_suicide', 'pill', 1.0)
        QBCore.Functions.Notify(Lang:t('consumables.canceled'), 'error')
    end)
...
-- Client-side script to apply armor when using LSD
RegisterNetEvent('consumables:client:lsd', function()
    local ped = PlayerPedId()
    local currentArmor = GetPedArmour(ped)

    QBCore.Functions.Progressbar('use_lsd', Lang:t('consumables.lsd_progress'), 5000, false, true, {
        disableMovement = false,
        disableCarMovement = false,
        disableMouse = false,
        disableCombat = true,
    }, {
        animDict = 'switch@trevor@trev_smoking_meth',  -- Using the same dict as meth for example
        anim = 'trev_smoking_meth_loop',
        flags = 49,
    }, {}, {}, function() -- Done
        StopAnimTask(ped, 'switch@trevor@trev_smoking_meth', 'trev_smoking_meth_loop', 1.0)
        
        -- Set armor to 100% directly
        SetPedArmour(ped, 100)
        QBCore.Functions.Notify("Armor increased to full!", "success")
