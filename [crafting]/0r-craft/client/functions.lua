    ...
    for key, value in pairs(_shops) do
        if value.active then
            ...
            if isObjectCreated then
                local _jobs = value.jobs
                if _jobs and type(_jobs) ~= "table" then
                    _jobs = { _jobs }
                end
            ...
            while true do
                ...
                if not self.isTheShopOpen then
                    ...
                    for key, item in pairs(items) do
                        local withinDistance = #(playerCoord - vector3(item.coords.x, item.coords.y, item.coords.z)) <=
                        item.distance
                        local jobValid = item.jobs == nil or item.jobs[job]

                        if withinDistance and jobValid then
...
function Koci.Client.Craft:LoadWeaponOnTable(weapon)
    ...
    if Koci.ResmonLib then
        ...
        local _rot = self.openedShop.object.rotation
        ...
        if weaponMetadata then
            ...
            elseif Config.FrameWork == "qb" then
                ...
                if type(item_components) == "table" and #item_components > 0 then
                    ...
                    for _, iComponent in pairs(item_components) do
                        for _, cWeaponComponent in pairs(cWeapon.components) do
                            local componentHash = type(iComponent) == "table" and iComponent.component or iComponent
...
function Koci.Client.Craft:RemoveQueueToTable(queue)
    for i = #self.completedCraftingQueues, 1, -1 do
        if self.completedCraftingQueues[i].db_index == queue.db_index then
