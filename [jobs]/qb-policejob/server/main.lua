...
QBCore.Commands.Add("depot", Lang:t("commands.depot"), {{name = "price", help = Lang:t('info.impound_price')}}, false, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if (Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty) or 
       (Player.PlayerData.job.type == "ems" and Player.PlayerData.job.onduty) or 
       (Player.PlayerData.job.type == "mechanic" and Player.PlayerData.job.onduty) then
            ...
            if biller.PlayerData.citizenid ~= billed.PlayerData.citizenid then
                if amount and amount > 0 then
                    if billed.Functions.RemoveMoney('bank', amount, "paid-fine") then
                        ...
                        exports['qb-management']:AddMoney(biller.PlayerData.job.name, amount*0.6)
                        biller.Functions.AddMoney('bank', amount*0.4, "paid-fine")
                    elseif billed.Functions.RemoveMoney('cash', amount, "paid-fine") then
                        TriggerClientEvent('QBCore:Notify', source, Lang:t("info.fine_issued"), 'success')
                        TriggerClientEvent('QBCore:Notify', billed.PlayerData.source, Lang:t("info.received_fine"))
                        exports['qb-management']:AddMoney(biller.PlayerData.job.name, amount*0.6)
                        biller.Functions.AddMoney('cash', amount*0.4, "paid-fine")
                    else
                        MySQL.Async.insert('INSERT INTO phone_invoices (citizenid, amount, society, sender, sendercitizenid) VALUES (?, ?, ?, ?, ?)',{billed.PlayerData.citizenid, amount, biller.PlayerData.job.name, biller.PlayerData.charinfo.firstname, biller.PlayerData.citizenid}, function(id)
                            if id then
                                TriggerClientEvent('yseries:client:AcceptorDenyInvoice', billed.PlayerData.source, id, biller.PlayerData.charinfo.firstname, biller.PlayerData.job.name, biller.PlayerData.citizenid, amount, GetInvokingResource())
                            end
                        end)
                        TriggerClientEvent('yseries:RefreshPhone', billed.PlayerData.source)
                    end
                else
...
RegisterNetEvent('police:server:SendTrackerLocation', function(coords, requestId)
    ...
    TriggerClientEvent("yseries:client:addPoliceAlert", requestId, alertData)
end)

-- Helper function to send job-specific notifications and map blips
-- Helper function to send job-specific notifications and map blips
local function sendJobAlert(job, commandName, alertTitle, alertMessage)
    QBCore.Commands.Add(commandName, alertTitle, {{name='message', help='Provide the alert message'}}, false, function(source, args)
        local src = source
        local message = args[1] and table.concat(args, " ") or 'A general alert was triggered!'
        ...
        for _, v in pairs(players) do
            if v and v.PlayerData.job.name == job and v.PlayerData.job.onduty then
                -- Trigger BraveNotify system
                TriggerClientEvent('QBCore:Notify', v.PlayerData.source, {text = message, caption = alertTitle}, 'info', 10000)

                -- Send map blip with job type
                TriggerClientEvent('custom_alerts:createBlip', v.PlayerData.source, coords, alertTitle, job)
            end
        end
    end)
end
...
--             TriggerClientEvent("yseries:client:addPoliceAlert", v.PlayerData.source, alertData)
...
RegisterNetEvent('police:server:EscortPlayer', function(playerId)
    ...
    local playerCoords = GetEntityCoords(playerPed)
    local targetCoords = GetEntityCoords(targetPed)
    if #(playerCoords - targetCoords) > 4.5 then return end
...
RegisterNetEvent('police:server:PutPlayerInVehicle', function(playerId)
    ...
    if EscortPlayer.PlayerData.metadata["ishandcuffed"] or EscortPlayer.PlayerData.metadata["server:policealert"] or EscortPlayer.PlayerData.metadata["isdead"] or EscortPlayer.PlayerData.metadata["inlaststand"] then
