				...
				if distance < 1.5 then
					...
					if(IsControlJustReleased(1, 38))then
						...
						if (tmp_action.type == "cleaning") then
							...
							local broomSpawn = CreateObject(GetHashKey("prop_tool_broom"), cSCoords.x, cSCoords.y, cSCoords.z, 1, 1, 1)
							local netid = ObjToNet(broomSpawn)
						
							RequestAnimDict("anim@amb@drug_field_workers@rake@male_a@idles")
							while not HasAnimDictLoaded("anim@amb@drug_field_workers@rake@male_a@idles") do
								Citizen.Wait(100)
							end
						
							TaskPlayAnim(PlayerPedId(), "anim@amb@drug_field_workers@rake@male_a@idles", "idle_b", 8.0, -8.0, -1, 49, 0, false, false, false)
						
							AttachEntityToEntity(broomSpawn, GetPlayerPed(PlayerId()), GetPedBoneIndex(GetPlayerPed(PlayerId()), 28422),
												 -0.0100, 0.0400, -0.0300, 0.0, 0.0, 0.0, true, true, false, true, 1, true)
						...
						if (tmp_action.type == "gardening") then
							...
							TaskPlayAnim(PlayerPedId(), "anim@amb@drug_field_workers@rake@male_b@base", "base", 8.0, -8.0, -1, 49, 0, false, false, false)
...
RegisterCommand("comserv", function()
 ...
 result = {
  ...
  if not qbinput1 then return end
  if not qbinput1.id then return end
