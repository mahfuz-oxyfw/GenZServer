...
local function SendNotify(msg,type,time,title)
    if NotifyType == nil then print("qb-smoking: NotifyType Not Set in Config.CoreSettings.Notify.Type!") return end
...
--open cig packs
RegisterNetEvent('qb-smoking:client:OpenPack', function(itemName)
    if busy then
        SendNotify("You Are Already Doing Something!", 'error', 2000)
    else
        QBCore.Functions.TriggerCallback('qb-smoking:get:CigPacks', function(HasItems)  
            if HasItems then
                busy = true
                LockInventory(true)
                if lib.progressCircle({ 
                    ...
                    busy = false
                    LockInventory(false)
                    TriggerServerEvent("qb-smoking:server:OpenPack", itemName)
...
--smoke cig
RegisterNetEvent('qb-smoking:client:SmokeCig', function()
    local playerPed = PlayerPedId()
    if busy then
        SendNotify("You Are Already Doing Something!", 'error', 2000)
    else
        QBCore.Functions.TriggerCallback('qb-smoking:get:Cigs', function(HasItems)  
            if HasItems then
                busy = true
                LockInventory(true)
                if lib.progressCircle({ 
                    ...
                    TriggerServerEvent('qb-smoking:server:SmokeCig')
                    ...
                    -- Change stress removal to 10%
                    TriggerServerEvent(Config.CoreSettings.EventNames.HudStatus, 10)  
                    busy = false
                    LockInventory(false)
                else 
                    busy = false
...
--smoke vape
RegisterNetEvent('qb-smoking:client:SmokeVape', function()
    local playerPed = PlayerPedId()
    if busy then
        SendNotify("You Are Already Doing Something!", 'error', 2000)
    else
        QBCore.Functions.TriggerCallback('qb-smoking:get:Vape', function(HasItems)  
            if HasItems then
                busy = true
                LockInventory(true)
                if lib.progressCircle({ 
                    ...
                    TriggerServerEvent('qb-smoking:server:SmokeVape')
                    ...
                    -- Change stress removal to 30%
                    TriggerServerEvent(Config.CoreSettings.EventNames.HudStatus, 30)
                    busy = false
                    LockInventory(false)
                else 
                    busy = false
...
--open smoking shop
RegisterNetEvent("qb-smoking:client:openShop", function()
    if InvType == 'qb'then
		TriggerServerEvent('qb-smoking:server:openShop')
