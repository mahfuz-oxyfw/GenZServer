...
RegisterNetEvent('arsh:client:removeBleed',function()
    ResetPartial2()
end)
...
RegisterNetEvent('qb-ambulancejob:checkin', function()
    ...
    if currentTime - lastCheckinTime < cooldownTime then
        QBCore.Functions.Notify("You must wait before checking in again.", "error")
        return
    end
    lastCheckinTime = currentTime
    if Config.MinimalDoctors > 0 and doctorCount >= Config.MinimalDoctors then
        print("Triggering SendDoctorAlert")
        TriggerServerEvent("hospital:server:SendDoctorAlert")
    else
        print("Proceeding with check-in")
...
if Config.UseTarget then
    CreateThread(function()
        ...
        for k, v in pairs(Config.Locations["checking"]) do
            exports['qb-target']:AddBoxZone("achecking"..k, vector3(v.x, v.y, v.z), 3.5, 2, {
                name = "achecking"..k,
                heading = -72,
                debugPoly = false,
                minZ = v.z - 2,
                ...
                options = {
                    {
                        ...
                        label = "Check In",
                        action = function()
                            if GetGameTimer() - lastUseTime >= 10000 and arshh then
                                arshh = false
                                ...
                                QBCore.Functions.Notify('You must wait before using this again!', 'error')
                            end
                        end
                    }
                },
                ...
                options = {
                    {
                        ...
                        label = "Layin Bed",
                        action = function()
                            if GetGameTimer() - lastUseTime >= 10000 then
                                lastUseTime = GetGameTimer()
                                TriggerEvent("qb-ambulancejob:beds")
                            else
                                TriggerEvent('qb-core:Notify', 'You must wait before using this again!', 'error')
                            end
                        end
                    }
                },
