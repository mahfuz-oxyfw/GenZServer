...
-- Server-side script to make LSD useable and to handle its removal
QBCore.Functions.CreateUseableItem('lsd', function(source)
    local Player = QBCore.Functions.GetPlayer(source)
    if Player then
        -- Check if the player's armor is already at the maximum before using the item
        if Player.PlayerData.metadata['armor'] >= 100 then
            TriggerClientEvent('QBCore:Notify', source, "Your armor is already full!", "error")
        else
            -- Only remove the item if it's actually going to be used
