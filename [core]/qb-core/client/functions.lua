...
function QBCore.Functions.Notify(text, texttype, length)
    if type(text) == "table" then
        ...
        if BraveNotify == false then
            ...
            local BraveType = 'info'
            if texttype == 'success' then
                BraveType = 'success'
            elseif texttype == 'error' then
                BraveType = 'error'
            end
            TriggerEvent('BraveNotify:Notify', BraveType, caption, ttext, length)
        end
    else
        texttype = texttype or 'primary'
        length = length or 5000
        if BraveNotify == false then
            ...
            TriggerEvent('BraveNotify:Notify', BraveType, "Notify", text, length)
        end
    end
end
