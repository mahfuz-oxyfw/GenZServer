...
lib.addCommand('combat', {
    ...
    TriggerClientEvent("gz-scoreboard:show", source)
end)
...
RegisterNetEvent("gz-scoreboard:server:banktrigger", function(status)
    Config.Robberies[1].status = status
    TriggerEvent("mm_robbery:server:sync", 30)
end)

RegisterNetEvent("gz-scoreboard:server:jewellerytrigger", function(status)
    Config.Robberies[2].status = status
    TriggerEvent("mm_robbery:server:sync", 30)
end)

RegisterNetEvent("gz-scoreboard:server:pacifictrigger", function(status)
