...
RegisterNetEvent('inventory:server:UseItemSlot', function(slot)
    ...
    if itemData then
        ...
        local itemNameLower = string.lower(itemData.name)
        local isRestricted = false
        for _, restrictedItem in ipairs(Config.RestrictedItems) do
            if itemNameLower == string.lower(restrictedItem) then
                isRestricted = true
                break
            end
        end

        if isRestricted and Player.PlayerData.job.name ~= "police" then
            TriggerClientEvent("QBCore:Notify", src, "You cannot use this item!", "error")
            return
        end
...
function ConvertQuality(item)
    local StartDate = tonumber(item.created) or 0
    local DecayRate = QBCore.Shared.Items[item.name:lower()] and QBCore.Shared.Items[item.name:lower()]["decay"] or 0.0
    DecayRate = tonumber(DecayRate) or 0.0
    local TimeExtra = math.ceil((TimeAllowed * DecayRate))
    if TimeExtra <= 0 then
        TimeExtra = 1
    end
    local percentDone = 100
    if StartDate > 0 and DecayRate > 0 then
        percentDone = 100 - math.ceil((((os.time() - StartDate) / TimeExtra) * 100))
    end
