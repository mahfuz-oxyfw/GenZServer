...
RegisterNetEvent('qb-bossmenu:client:OpenMenu', function()
    ...
    local bossMenu = {
        {
            header = LangManagement:t("headers.bsm").. string.upper(PlayerJob.label),
            icon = "fa-solid fa-circle-info",
            isMenuHeader = true,
        },
        {
            header = LangManagement:t("body.manage"),
            txt = LangManagement:t("body.managed"),
        ...
        {
            header = LangManagement:t("body.hire"),
            txt = LangManagement:t("body.hired"),
        ...
        {
            header = LangManagement:t("body.storage"),
            txt = LangManagement:t("body.storaged"),
        ...
        {
            header = LangManagement:t("body.outfits"),
            txt = LangManagement:t("body.outfitsd"),
        ...
        {
            header = LangManagement:t("body.money"),
            txt = LangManagement:t("body.moneyd"),
    ...
    bossMenu[#bossMenu + 1] = {
        header = LangManagement:t("body.exit"),
...
RegisterNetEvent('qb-bossmenu:client:employeelist', function()
    local EmployeesMenu = {
        {
            header = LangManagement:t("body.mempl").. string.upper(PlayerJob.label),
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info",
        },
    }
    QBCore.Functions.TriggerCallback('qb-bossmenu:server:GetEmployees', function(cb)
        ...
        EmployeesMenu[#EmployeesMenu + 1] = {
            header = LangManagement:t("body.return"),
            icon = "fa-solid fa-angle-left",
            params = {
...
RegisterNetEvent('qb-bossmenu:client:ManageEmployee', function(data)
    local EmployeeMenu = {
        {
            header = LangManagement:t("body.mngpl").. data.player.name .. " - " .. string.upper(PlayerJob.label),
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info"
        },
    }
    for k, v in pairs(QBCore.Shared.Jobs[data.work.name].grades) do
        EmployeeMenu[#EmployeeMenu + 1] = {
            header = v.name,
            txt =  LangManagement:t("body.grade") .. k,
            params = {
                isServer = true,
    ...
    EmployeeMenu[#EmployeeMenu + 1] = {
        header = LangManagement:t("body.fireemp"),
...
RegisterNetEvent('qb-bossmenu:client:HireMenu', function()
    local HireMenu = {
        {
            header = LangManagement:t("body.hireemp").. string.upper(PlayerJob.label),
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info",
        },
    }
    QBCore.Functions.TriggerCallback('qb-bossmenu:getplayers', function(players)
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMenu[#HireMenu + 1] = {
                    header = v.name,
                    txt = LangManagement:t("body.cid").. v.citizenid .. " - ID: " .. v.sourceplayer,
...
RegisterNetEvent('qb-bossmenu:client:SocietyMenu', function()
    QBCore.Functions.TriggerCallback('qb-bossmenu:server:GetAccount', function(cb)
        local SocietyMenu = {
            {
                header = LangManagement:t("body.balance").. comma_value(cb) .. " - " .. string.upper(PlayerJob.label),
                isMenuHeader = true,
                icon = "fa-solid fa-circle-info",
            },
            {
                header = LangManagement:t("body.deposit"),
                icon = "fa-solid fa-money-bill-transfer",
                txt = LangManagement:t("body.depositd"),
                params = {
            ...
            {
                header = LangManagement:t("body.withdraw"),
                icon = "fa-solid fa-money-bill-transfer",
                txt = LangManagement:t("body.withdrawd"),
                params = {
...
RegisterNetEvent('qb-bossmenu:client:SocetyDeposit', function(money)
    local deposit = exports['qb-input']:ShowInput({
        header = LangManagement:t("body.depositm").. money,
        submitText = LangManagement:t("body.submit"),
        inputs = {
            {
                type = 'number',
                isRequired = true,
                name = 'amount',
                text = LangManagement:t("body.amount")
            }
        }
    })
    if deposit then
...
RegisterNetEvent('qb-bossmenu:client:SocetyWithDraw', function(money)
    local withdraw = exports['qb-input']:ShowInput({
        header = LangManagement:t("body.withdrawm").. money,
        submitText = LangManagement:t("body.submit"),
        inputs = {
            {
                type = 'number',
...
CreateThread(function()
    if ConfigManagement.UseTarget then
        for job, zones in pairs(ConfigManagement.BossMenuZones) do
                    ...
                    options = {
                        {
                            ...
                            label = LangManagement:t("target.label"),
        ...
        while true do
            ...
            if PlayerJob then
                wait = 0
                for k, menus in pairs(ConfigManagement.BossMenus) do
                            ...
                            if #(pos - coords) < 5.0 then
                                ...
                                if #(pos - coords) <= 1.5 then
                                    ...
                                    if not shownBossMenu then
                                        exports['qb-core']:DrawText(LangManagement:t("drawtext.label"), 'left')
                                        shownBossMenu = true
                                    end
