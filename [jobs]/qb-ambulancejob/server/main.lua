...
RegisterNetEvent('hospital:server:SendToBed2', function(bedId, isRevive)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	TriggerClientEvent('hospital:client:SendToBed', src, bedId, Config.Locations["beds"][bedId], isRevive)
	TriggerClientEvent('hospital:client:SetBed', -1, bedId, true)
...
RegisterNetEvent('hospital:server:SetDeathStatus', function(isDead)
    ...
    if Player then
        ...
        TriggerClientEvent('cs:advradio:isDead', src, isDead)
        if isDead then
            TriggerEvent('cs:advradio:forceLeave', src) -- Use TriggerEvent for server-side events
        end
    end
end)
...
RegisterNetEvent('hospital:server:SetLaststandStatus', function(bool)
    ...
    if Player then
        ...
        TriggerClientEvent('cs:advradio:isDead', src, bool)
        if bool then
            TriggerEvent('cs:advradio:forceLeave', src) -- Use TriggerEvent for server-side events
        end
    end
...
RegisterNetEvent('hospital:server:RevivePlayer', function(playerId, isOldMan)
	...
	if Patient then
		if Player.PlayerData.job.name == "ambulance" or QBCore.Functions.HasItem(src, "medicalkit", 1) then
			if oldMan then
				if Player.Functions.RemoveMoney("cash", 2000, "revived-player") then
					Player.Functions.RemoveItem('medicalkit', 1)
					TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['medicalkit'], "remove")
					TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
				else
					TriggerClientEvent('QBCore:Notify', src, Lang:t('error.not_enough_money'), "error")
				end
			else
				if Patient.Functions.RemoveMoney("bank", 800 , "treatment") then
					Player.Functions.RemoveItem('medicalkit', 1)
					TriggerClientEvent('inventory:client:ItemBox', src, QBCore.Shared.Items['medicalkit'], "remove")
					TriggerClientEvent('hospital:client:Revive', Patient.PlayerData.source)
					Player.Functions.AddMoney("bank", 240 , "treatment")
					exports['qb-management']:AddMoney("ambulance", 560)
				end
			end
		else
...
RegisterNetEvent('hospital:server:resetHungerThirst', function()
    ...
    if hunger < 30 then
		hunger = 30
        Player.Functions.SetMetaData('hunger', 30)
    end

    if thirst < 30 then
		thirst = 30
        Player.Functions.SetMetaData('thirst', 30)
    end
...
RegisterNetEvent("ems:server:PutPlayerOnBed", function(t)
	if t == -1 then return end
    TriggerClientEvent('qb-ambulancejob:beds2', t)
end)