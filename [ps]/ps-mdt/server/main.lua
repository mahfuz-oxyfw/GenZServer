...
local ClockinWebhook = 'https://discord.com/api/webhooks/1330830404868902912/-1HP8IH8Bg7ljdBuXYBYoTcuoRobl2y8SeOzLR64lNgpNZ0-wDa79Ka0iNO9i9Rbid4R'

-- Incident and Incident editting. Add a Discord webhook.
-- Incident Author, Title, and Report will display in webhook post.
local IncidentWebhook = 'https://discord.com/api/webhooks/1330830522083049536/_R5qvHNGyrK_xRcuD7lOKOm_vLr1GO7vqOYVqJQL_hZ323XrilzQzeOv3VKJKZZ-VK2g'
--------------------------------
...
QBCore.Commands.Add("mdtleaderboard", "Show MDT leaderboard", {}, false, function(source, args)
    ...
    sendToDiscord(16753920, "MDT Leaderboard", leaderboard_message, "ps-mdt | Made by ProfEX")
                        ...
                        MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = infoResult }, function(result)
                            if result and #result > 0 then
                                ...
                                for i=1, #associated do
                                    ...
                                    sendIncidentToDiscord(3989503, "MDT Incident Report", message, "ps-mdt | Made by ProfEX", associatedData)                                
                                end
                            else
                ...
                MySQL.Async.fetchAll('SELECT `author`, `title`, `details` FROM `mdt_incidents` WHERE `id` = @id', { ['@id'] = id }, function(result)
                    if result and #result > 0 then
                        ...
                        for i=1, #associated do
                            ...
                            sendIncidentToDiscord(16711680, "MDT Incident Report has been Updated", message, "ps-mdt | Made by ProfEX", associatedData)
                        end
                    else
...
QBCore.Functions.CreateUseableItem('mdt', function(source)
    TriggerClientEvent('mdt:client:UseMDTItem', source)
end)
...
function PaycheckInterval()
    QBCore.Config.Money.PayCheckSociety = true
            ...
            if Player then
                ...
                if jobData then
                    ...
                    if jobData.onduty then
                        if jobName and gradeLevel then
                            local payment = QBCore.Shared.Jobs[jobName] 
                                and QBCore.Shared.Jobs[jobName]['grades'] 
                                and QBCore.Shared.Jobs[jobName]['grades'][gradeLevel] 
                                and QBCore.Shared.Jobs[jobName]['grades'][gradeLevel].payment
                                ...
                                if QBCore.Config.Money.PayCheckSociety then
                                    ...
                                    if account ~= nil then
                                        if account < payment then
                                            TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "Company too poor!", 'error')
                                        else
                                            Player.Functions.AddMoney('bank', payment, 'paycheck')
                                            exports['qb-management']:RemoveMoney(jobName, payment, 'Employee Paycheck')
                                            TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "Received Paycheck! $" .. payment)
                                        end
                                    else
                                        Player.Functions.AddMoney('bank', payment, 'paycheck')
                                        TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "Received Paycheck! $" .. payment)
                        ...
                        TriggerClientEvent('QBCore:Notify', Player.PlayerData.source, "You must be on duty to receive your paycheck!", 'error')
                    end
                end
            end
        end
