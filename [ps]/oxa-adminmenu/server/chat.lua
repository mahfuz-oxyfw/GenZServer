...
-- Staff Chat
RegisterNetEvent('oxa-adminmenu:server:sendMessageServer', function(message, citizenid, fullname)
...
lib.callback.register('oxa-adminmenu:callback:GetMessages', function()
    if not CheckPerms(source, 'mod') then return {} end
    return messages
end)
