-- Freeze Player
local frozen = false
RegisterNetEvent('oxa-adminmenu:server:FreezePlayer', function(data, selectedData)
...
-- Drunk Player
RegisterNetEvent('oxa-adminmenu:server:DrunkPlayer', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    ...
    TriggerClientEvent('oxa-adminmenu:client:InitiateDrunkEffect', target)
    QBCore.Functions.Notify(src,
        locale("playerdrunk",
            Player.PlayerData.charinfo.firstname ..
            " " .. Player.PlayerData.charinfo.lastname .. " | " .. Player.PlayerData.citizenid), 'Success', 7500)
