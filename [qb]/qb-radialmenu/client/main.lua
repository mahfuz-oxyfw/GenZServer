...
local function UnlockAllVehicleDoors(vehicle)
    SetVehicleDoorsLocked(vehicle, 1)
    for door = 0, GetNumberOfVehicleDoors(vehicle) - 1 do
        SetVehicleDoorShut(vehicle, door, false)
    end
end

local function GivePlayerVehicleKeys(vehicle)
    QBCore.Functions.Notify("You have received the vehicle keys.", "success")
    TriggerEvent('qb-vehiclekeys:client:AddKeys', QBCore.Functions.GetPlate(vehicle))
end

RegisterNetEvent('client:unlockNearestVehicle', function()
    local playerData = QBCore.Functions.GetPlayerData()
    if playerData.job and playerData.job.type == 'leo' then
        local playerPed = PlayerPedId()
        local coords = GetEntityCoords(playerPed)
        local nearestVehicle = QBCore.Functions.GetClosestVehicle(coords)
        if nearestVehicle and DoesEntityExist(nearestVehicle) then
            QBCore.Functions.Progressbar("unlock_vehicle", "Unlocking Vehicle...", 2000, false, true, {
                ...
                UnlockAllVehicleDoors(nearestVehicle)
                if IsPedInAnyVehicle(playerPed, false) then
                    local currentVehicle = GetVehiclePedIsIn(playerPed, false)
                    GivePlayerVehicleKeys(currentVehicle)
                else
                    QBCore.Functions.Notify("Vehicle doors unlocked.", "success")
                end
            end, function()
                QBCore.Functions.Notify("Action canceled.", "error")
            end)
        else
            QBCore.Functions.Notify("No nearby vehicle found.", "error")
        end
    else
        QBCore.Functions.Notify("You do not have permission to perform this action.", "error")
    end
end)

function GetVehicleClosestSeat(vehicle, coords)
    for i = -1, GetVehicleMaxNumberOfPassengers(vehicle) - 1 do
        if IsVehicleSeatFree(vehicle, i) then
            return i
        end
    end
    return -1 -- No free seat found
end


RegisterNetEvent('admin:unlockNearestVehicle', function()
    local playerPed = PlayerPedId()
    local coords = GetEntityCoords(playerPed)
    local nearestVehicle = QBCore.Functions.GetClosestVehicle(coords)
    if nearestVehicle and DoesEntityExist(nearestVehicle) then
        UnlockAllVehicleDoors(nearestVehicle)
        TaskWarpPedIntoVehicle(PlayerPedId(), nearestVehicle, GetVehicleClosestSeat(nearestVehicle, coords))
        if IsPedInAnyVehicle(playerPed, false) then
            local currentVehicle = GetVehiclePedIsIn(playerPed, false)
            GivePlayerVehicleKeys(currentVehicle)
        else
