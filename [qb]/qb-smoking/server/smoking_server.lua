...
QBCore.Functions.CreateUseableItem("cigs", function(source, item)
    TriggerClientEvent("qb-smoking:client:SmokeCig", source)
end)

QBCore.Functions.CreateUseableItem("vape", function(source, item)
    TriggerClientEvent("qb-smoking:client:SmokeVape", source)
end)
...
for k,_ in pairs(Config.SmokingItems) do
    QBCore.Functions.CreateUseableItem(k, function(source, item)
        TriggerClientEvent("qb-smoking:client:OpenPack", source, item.name)
    end)
end

--smoke cig
RegisterNetEvent('qb-smoking:server:OpenPack', function(item)
    local src = source
...
QBCore.Functions.CreateCallback('qb-smoking:get:CigPacks', function(source, cb)
    local src = source
...
QBCore.Functions.CreateCallback('qb-smoking:get:Cigs', function(source, cb)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
QBCore.Functions.CreateCallback('qb-smoking:get:Vape', function(source, cb)
    local src = source
    local Ply = QBCore.Functions.GetPlayer(src)
...
RegisterNetEvent('qb-smoking:server:SmokeVape', function()
    local src = source
...
RegisterNetEvent('qb-smoking:server:SmokeCig', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if not Player then return end
    if InvType == 'qb' then
...
RegisterNetEvent('qb-smoking:server:openShop', function()
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local smokingShop = {
...
AddEventHandler('onResourceStart', function(resourceName)
    if (GetCurrentResourceName() == resourceName) then
        if InvType == 'ox' then
            print('^5--<^3!^5>-- ^7| qb-smoking |^5 ^5--<^3!^5>--^7')
