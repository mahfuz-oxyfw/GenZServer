...
RegisterServerEvent('ps-drugprocessing:process_lsa', function()
    ...
    if Player.Functions.RemoveItem('chemicals', 1) then
        local addedLSA = Player.Functions.AddItem('lsa', 1)
        local addedThionylChloride = Player.Functions.AddItem('thionyl_chloride', 1)

        if addedLSA and addedThionylChloride then 
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items['chemicals'], "remove")
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items['lsa'], "add")
            TriggerClientEvent("inventory:client:ItemBox", src, QBCore.Shared.Items['thionyl_chloride'], "add")
            TriggerClientEvent('QBCore:Notify', src, Lang:t("success.lsa_thionyl"), "success")
        else
            -- If either item couldn't be added, return the chemicals
            Player.Functions.AddItem('chemicals', 1)
            -- Determine which item failed and notify accordingly
            if not addedLSA then
                TriggerClientEvent('QBCore:Notify', src, Lang:t("error.no_lsa_space"), "error")
            end
            if not addedThionylChloride then
                TriggerClientEvent('QBCore:Notify', src, Lang:t("error.no_thionyl_space"), "error")
            end
        end
    else
        TriggerClientEvent('QBCore:Notify', src, Lang:t("error.no_chemicals"), "error")
