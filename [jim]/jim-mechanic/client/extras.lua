...
RegisterNetEvent('jim-mechanic:vehFailure:RepairVehicle', function(data)
    ...
    if not DoesEntityExist(vehicle) or lockedCar(vehicle) then return end
    local enginePos = GetWorldPositionOfEntityBone(vehicle, GetEntityBoneIndexByName(vehicle, "engine"))
    if #(coords - enginePos) >= 2.0 then return end
    TaskTurnPedToFaceEntity(playerPed, vehicle, 1000)
    Wait(1000)
    SetVehicleDoorOpen(vehicle, 4, false, false)
    QBCore.Functions.Progressbar("veh_failure_repair", Loc[Config.Lan]["repair"].repairing, 10000, false, true, { disableMovement = true, disableCarMovement = true, disableMouse = false, disableCombat = false }, { animDict = "mini@repair", anim = "fixing_a_player", flags = 16 }, {}, {}, function()
        Wait(1000)
        ...
        if full then
            ...
            if GetResourceState("qs-advancedgarages"):find("start") then
                exports["qs-advancedgarages"]:RepairNearestVehicle()
            end
        else
            local currentEngHealth = GetVehicleEngineHealth(vehicle)
            local currentBodyHealth = GetVehicleBodyHealth(vehicle)
            SetVehicleEngineHealth(vehicle, currentEngHealth + math.random(300,400))
            SetVehicleBodyHealth(vehicle, currentBodyHealth + 200)
        end
