...
local function preparePlayers()
    currentPlayers = {}
    Wait(100)
    currentPlayers = lib.callback.await('oxa-adminmenu:callback:GetPlayers')
end

-- Toggle Blips and Names events
RegisterNetEvent('oxa-adminmenu:client:toggleBlips', function(data)
...
RegisterNetEvent('oxa-adminmenu:client:toggleNames', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Mute Player
RegisterNetEvent("oxa-adminmenu:client:MutePlayer", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Remove Stress
RegisterNetEvent('oxa-adminmenu:client:removeStress', function(data)
