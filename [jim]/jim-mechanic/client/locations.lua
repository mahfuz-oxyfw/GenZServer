		...
		if v.clockin then
			for l, b in pairs(v.clockin) do
				if type(b) ~= "boolean" then
					...
					exports['qb-target']:AddBoxZone("MechClock: "..k..l, b.coords.xyz, (b.w or 1.45), (b.d or 1.4), { name="MechClock: "..k..l, heading = b.coords.w, debugPoly=Config.Debug, minZ=b.coords.z-0.1, maxZ=b.coords.z+0.4 },
						{ options = {
...
RegisterNetEvent('jim-mechanic:client:Store:Menu', function(data)
    ...
    local menu = {}
	if Player.job.grade.level == 0 then triggerNotify(nil, 'Not Authorized!', "error") return end
    table.insert(menu, {
        header = Loc[Config.Lan]["stores"].tools,
        txt = "",
        params = { event = "jim-mechanic:client:Store", args = { id = 1, job = data.job } }
    })

    if grade >= 1 then
        table.insert(menu, {
            header = Loc[Config.Lan]["stores"].cosmetic,
            txt = "",
            params = { event = "jim-mechanic:client:Store", args = { id = 3, job = data.job } }
        })
    end

    if grade >= 2 then
        table.insert(menu, {
            header = Loc[Config.Lan]["stores"].perform,
            txt = "",
            params = { event = "jim-mechanic:client:Store", args = { id = 2, job = data.job } }
        })
    end
...
Config.Locations = {
	...
	{	job = "mechanic",
		zones = {
			vector2(2764.6030273438, 3506.9841308594),
			vector2(2735.408203125, 3436.7133789062),
			vector2(2644.8059082031, 3475.7312011719),
			vector2(2659.5314941406, 3510.6677246094),
			vector2(2673.5141601562, 3504.8403320312),
			vector2(2687.8842773438, 3538.9702148438)
		},
		autoClock = { enter = false, exit = false, },
		stash = { { coords = vector4(2673.27, 3488.9, 56.63, 170.71), w = 2, d = 3 }, },
		store = { { coords = vector4(2666.04, 3503.68, 56.67, 109.92), w = 1.5, d = 13 }, },
		crafting = { { coords = vector4(2739.59, 3499.54, 55.68, 71.49), w = 2.5, d = 0.7 }, },
		clockin = { { coords = vector4(2743.99, 3466.8, 55.62, 71.49), prop = false, }, },
		--manualRepair = { { coords = vector4(69.07, -1759.16, 29.66, 50.56), prop = true, } },
		--[[ garage = { spawn = vec4(-49.14, -1031.25, 27.76, 339.67),
					out = vec4(-42.38, -1035.11, 28.52, 68.01),
					list = { "towtruck", "panto", "slamtruck", "cheburek", "utillitruck3" },
					prop = true, }, ]]
		payments = { coords = vector4(-200.68, -1314.53, 31.08, 0.0),     --2743.2, 3467.38, 55.60, 335.41
			...
			prop = true, },
		blip = vector3(2718.13, 3489.7, 55.73),
