...
local visible = true
RegisterNetEvent('oxa-adminmenu:client:ToggleInvisible', function(data)
...
-- God Mode
local godmode = false
RegisterNetEvent('oxa-adminmenu:client:ToggleGodmode', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    godmode = not godmode
...
-- Cuff/Uncuff
RegisterNetEvent('oxa-adminmenu:client:ToggleCuffs', function(player)
...
RegisterNetEvent('oxa-adminmenu:client:setInfiniteAmmo', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent('oxa-adminmenu:client:ToggleCoords', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Set Ammo
RegisterNetEvent('oxa-adminmenu:client:SetAmmo', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent('oxa-adminmenu:client:ToggleDev', function(dataKey)
    ...
    TriggerEvent('oxa-adminmenu:client:ToggleCoords', dataKey)  -- toggle Coords
    TriggerEvent('oxa-adminmenu:client:ToggleGodmode', dataKey) -- Godmode
...
-- Set Ped
RegisterNetEvent("oxa-adminmenu:client:setPed", function(pedModels)
