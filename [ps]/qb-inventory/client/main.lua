...
local function GetAttachmentThresholdItems()
	...
	if not Config.AttachmentCrafting then
		return {}
	end
	if not PlayerData.metadata or not PlayerData.metadata["attachmentcraftingrep"] then
		return {}
	end
	local items = {}
	for k, v in pairs(Config.AttachmentCrafting) do
		if v and v.threshold and PlayerData.metadata["attachmentcraftingrep"] >= v.threshold then
			items[k] = v
		end
	end
	return items
