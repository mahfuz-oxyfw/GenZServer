...
local prisonCoords = vector3(1690.8124, 2593.4309, 45.5613)
local radius = 100.0

local function CheckPlayerJailStatus()
    local PlayerData = QBCore.Functions.GetPlayerData()
	if PlayerData then
    	if PlayerData and PlayerData.metadata and PlayerData.metadata['injail'] and PlayerData.metadata['injail'] > 0 then
    	    local playerPed = PlayerPedId()
    	    local playerCoords = GetEntityCoords(playerPed)
    	    local distance = #(playerCoords - prisonCoords)
    	    if distance > radius then
    	        TriggerEvent('prison:client:Enter', PlayerData.metadata['injail'])
    	        QBCore.Functions.Notify("Do not try to escape!", "error")
    	    end
    	end
	end
end
