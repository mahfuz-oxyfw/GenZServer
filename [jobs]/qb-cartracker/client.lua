...
RegisterNetEvent('QBCore:Client:OnPlayerLoaded', function ()
    ...
    if playerData.job.name == 'police' then
        ...
        TriggerServerEvent("qb-cartracker:getActivePlates")
    end
end)

RegisterNetEvent("qb-cartracker:updateTimer", function(plate)
    time_out[plate] = time_out[nil]
end)

RegisterNetEvent("qb-cartracker:updateActivePlate", function(plate)
...
RegisterNetEvent("qb-cartracker:getActivePlates", function(plates)
    time_out = plates
    for v,k in pairs(time_out) do
        checkVehicle(v)
    end
...
RegisterNetEvent('qb-cartracker:plate', function(plate)
    checkVehicle(plate)
end)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == data.plate then 
            ...
            if nCheck >= Config.maxTracker then 
                ...
                SetNuiFocus(false)
                TriggerServerEvent("qb-cartracker", data.plate)
            end
        else 
            miss = miss + 1 
        end 
...
RegisterNUICallback("removeSearch", function(data, cb)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == data.plate then 
            TriggerServerEvent("qb-cartracker:removeActivePlate", data.plate)
            SendNUIMessage({
                type = "ui",
                display = false
              })
...
function checkVehicle(plate)
    ...
    for i=1, #vehicle, 1 do 
        ...
        if vehicleProps.plate == plate then 
            TriggerServerEvent("qb-cartracker:setActivePlates", plate)
...
function triggerTimer(plate)
    TriggerServerEvent("qb-cartracker:triggerTimer", plate)
end
        ...
        CreateThread(function()
            while time_out[plate] == false do
                ...
                if DoesEntityExist(vehicle) then 
                    ...
                    time_out[plate] = time_out[nil]
                    TriggerServerEvent("qb-cartracker:removeActivePlate", plate)
