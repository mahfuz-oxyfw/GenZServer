...
local function presentAllowlistRequest(deferrals, playerId)
    local isThereAnyWhitelistedPlayer = (not hasFirstLoadFinished) and true or false -- If first load is not finished, pretend there is already a whitelisted player
...
RegisterNetEvent('playerConnecting', function(playerName, setKickReason, deferrals)
    ...
    if(not card) then
        local kickReasonText = GetCurrentResourceName() .. " not ready, try again in few minutes"
        setKickReason(kickReasonText)
        CancelEvent()
        deferrals.done(kickReasonText)

        return
    end

    if(not EasyAllowlist.getPlayerIdentifier(playerId)) then
        local kickReasonText = "Couldn't get your " .. config.mainIdentifier .. " identifier\n" .. getLocalizedText(missingIdentifierMessages[config.mainIdentifier])
        setKickReason(kickReasonText)
        CancelEvent()
        deferrals.done(kickReasonText)
