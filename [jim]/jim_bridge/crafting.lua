...
function openShop(data)
    ...
    elseif GetResourceState(QBInv):find("start") then
        if QBInvNew then
            ...
            TriggerServerEvent(Config.General.JimShops and "jim-shops:ShopOpen" or "qb-inventory:server:OpenInventory", "shop", data.items.label, data.items)
        end    
    else
        TriggerServerEvent(Config.General.JimShops and "jim-shops:ShopOpen" or "qb-inventory:server:OpenInventory", "shop", data.items.label, data.items)
    end
...
function openStash(data)
    ...
    elseif GetResourceState(PSInv):find("start") then
        if QBInvNew then
            ...
            TriggerEvent("qb-inventory:client:SetCurrentStash", data.stash)
            TriggerServerEvent("qb-inventory:server:OpenInventory", "stash", data.stash, data.stashOptions)
        end
    else
        TriggerEvent("qb-inventory:client:SetCurrentStash", data.stash)
        TriggerServerEvent("qb-inventory:server:OpenInventory", "stash", data.stash, data.stashOptions)
