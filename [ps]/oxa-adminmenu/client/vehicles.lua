...
-- Own Vehicle
RegisterNetEvent('oxa-adminmenu:client:Admincar', function(data)
    ...
    if sharedVehicles then
        TriggerServerEvent('oxa-adminmenu:server:SaveCar', props, sharedVehicles, hash, props.plate)
    else
...
-- Spawn Vehicle
RegisterNetEvent('oxa-adminmenu:client:SpawnVehicle', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Refuel Vehicle
RegisterNetEvent('oxa-adminmenu:client:RefuelVehicle', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Change plate
RegisterNetEvent('oxa-adminmenu:client:ChangePlate', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    ...
    if cache.vehicle then
        local AlreadyPlate = lib.callback.await("oxa-adminmenu:callback:CheckAlreadyPlate", false, plate)
        ...
        TriggerServerEvent('oxa-adminmenu:server:ChangePlate', plate, currentPlate)
        Wait(100)
...
RegisterNetEvent('oxa-adminmenu:client:ToggleVehDevMenu', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    if not cache.vehicle then return end
...
RegisterNetEvent('oxa-adminmenu:client:maxmodVehicle', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
RegisterNetEvent("oxa-adminmenu:client:SpawnPersonalVehicle", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
    ...
    lib.callback('oxa-adminmenu:server:GetVehicleByPlate', false, function(vehModel)
        vehicle = vehModel
    end, plate)
...
-- Get Vehicle Data
lib.callback.register("oxa-adminmenu:client:getvehData", function(vehicle)
