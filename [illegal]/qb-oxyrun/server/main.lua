...
RegisterServerEvent('DD-oxyrun:server:SetOccupied', function(route)
	Config.Routes[route].info.occupied = true
	TriggerClientEvent('DD-oxyrun:client:SetOccupied', -1, route)
end)

-- Starting Conditions

RegisterServerEvent('DD-oxyrun:server:startr', function()
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)

	if Player.PlayerData.money['cash'] >= Config.RunCost then
		TriggerClientEvent("DD-oxyrun:client:sendToOxy", src)
	else
		TriggerClientEvent('QBCore:Notify', source, Lang:t("error.you_dont_have_enough_money"), 'error')
	end
end)

RegisterServerEvent('DD-oxyrun:server:TakeMoney', function()
    local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	Player.Functions.RemoveMoney('cash', Config.RunCost)

	if Config.EnableCooldown then
        TriggerEvent('DD-oxyrun:server:coolout')
	end
end)

-- Cooldown

RegisterServerEvent('DD-oxyrun:server:coolout', function()
    Cooldown = true
...
QBCore.Functions.CreateCallback("DD-oxyrun:server:coolc",function(source, cb)
    if Cooldown then
        cb(true)
    else
        cb(false) 
...
QBCore.Functions.CreateCallback('DD-oxyrun:server:getCops', function(source, cb)
    local amount = 0
...
RegisterNetEvent("DD-oxyrun:server:sendCar")
AddEventHandler("DD-oxyrun:server:sendCar", function(Routes)
	if oxyStarted == false then
		TriggerClientEvent("DD-oxyrun:createOxyVehicle", source, Routes)
		
	end
end)

RegisterNetEvent("DD-oxyrun:server:deliver")
AddEventHandler("DD-oxyrun:server:deliver", function()
...
QBCore.Functions.CreateUseableItem('package', function(source) 
	TriggerClientEvent('DD-oxyrun:client:hasPackage', source) 
end) 


QBCore.Functions.CreateCallback('DD-oxyrun:server:hasPackage', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local hasPackage = Player.Functions.GetItemByName("package")
...
QBCore.Functions.CreateCallback('DD-oxyrun:server:canCarry', function(source, cb)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
