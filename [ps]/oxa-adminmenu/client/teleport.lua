...
-- Teleport to player
RegisterNetEvent('oxa-adminmenu:client:TeleportToPlayer', function(coords)
    lastCoords = GetEntityCoords(cache.ped)
    SetPedCoordsKeepVehicle(cache.ped, coords.x, coords.y, coords.z)
end)

-- Teleport to coords
RegisterNetEvent('oxa-adminmenu:client:TeleportToCoords', function(data, selectedData)
...
-- Teleport to Locaton
RegisterNetEvent('oxa-adminmenu:client:TeleportToLocation', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
...
-- Teleport back
RegisterNetEvent('oxa-adminmenu:client:TeleportBack', function(data)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(data.perms) then return end
