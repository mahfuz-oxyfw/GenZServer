...
-- custom notification blip
RegisterNetEvent('custom_alerts:createBlip', function(coords, title, job)
    local blip = AddBlipForCoord(coords.x, coords.y, coords.z)

    -- Customize blip based on job
    if job == 'police' then
        SetBlipSprite(blip, 42) -- Police badge icon
        SetBlipColour(blip, 1) -- Blue
    elseif job == 'ambulance' then
        SetBlipSprite(blip, 153) -- Ambulance icon
        SetBlipColour(blip, 1) -- Red
    elseif job == 'mechanic' then
        SetBlipSprite(blip, 380) -- Wrench icon
        SetBlipColour(blip, 5) -- Yellow
    else
        SetBlipSprite(blip, 1) -- Default
    ...
    -- Blip duration (e.g., 10 minutes)
    Citizen.CreateThread(function()
        Wait(60000) -- 1 minutes in milliseconds
        RemoveBlip(blip)
    end)
end)
