            ...
            for k, v in pairs(result) do
                ...
                if type == "depot" then
                    MenuGarageOptions[#MenuGarageOptions+1] = {
                        ...
                        params = {
                            event = "qb-garages:client:TakeOutDepotWithCooldown",
                            args = {
                                vehicle = v,
                    ...
                    MenuGarageOptions[#MenuGarageOptions+1] = {
                        ...
                        params = {
                            event = "qb-garages:client:TakeOutGarageWithCooldown",
                            args = {
                                vehicle = v,
                                vehicleModel = v.vehicle,
                                type = type,
...
RegisterNetEvent("qb-garages:client:TakeOutDepotWithCooldown", function(args)
    if canTakeOutVehicle then
        canTakeOutVehicle = false
        TriggerEvent("qb-garages:client:TakeOutDepot", args)
        Citizen.SetTimeout(10000, function()
            canTakeOutVehicle = true
        end)
    else
        QBCore.Functions.Notify("Please wait before taking out another vehicle.", "error", 5000)
    end
end)

RegisterNetEvent("qb-garages:client:TakeOutGarageWithCooldown", function(args)
    if canTakeOutVehicle then
        canTakeOutVehicle = false
