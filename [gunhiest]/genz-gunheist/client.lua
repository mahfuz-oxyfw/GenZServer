...
RegisterNetEvent('genz-gunheist:manipulateDrop')
AddEventHandler('genz-gunheist:manipulateDrop', function(dropId, dropData)
...
RegisterNetEvent('genz-gunheist:obtainDrops')
AddEventHandler('genz-gunheist:obtainDrops', function(serverDrops)
	activeDrops = serverDrops
end)

RegisterNetEvent('genz-gunheist:createBlipOnDrop')
AddEventHandler('genz-gunheist:createBlipOnDrop', function(dropId)
...
AddEventHandler('genz-gunheist:createBlipOnDrop', function(dropId)
    ...
    local radiusBlip = AddBlipForRadius(coords.x, coords.y, coords.z, 200.0)
    ...
    SetBlipSprite(pointBlip, 478)
    SetBlipScale(pointBlip, 0.9)
    SetBlipColour(pointBlip, 4)
    ...
    activeDrops[dropId].radiusBlip = radiusBlip
    activeDrops[dropId].pointBlip = pointBlip

    Citizen.SetTimeout(900000, function()
        if activeDrops[dropId] then
            if activeDrops[dropId].radiusBlip then
                RemoveBlip(activeDrops[dropId].radiusBlip)
            end
            if activeDrops[dropId].pointBlip then
                RemoveBlip(activeDrops[dropId].pointBlip)
            end
...
RegisterNetEvent('genz-gunheist:openCrate')
AddEventHandler('genz-gunheist:openCrate', function(crateObject)
...
Citizen.CreateThread(function()
	TriggerServerEvent('genz-gunheist:obtainDrops')
...
AddEventHandler('do', function(dropId)
	...
	TriggerEvent('genz-gunheist:openCrate', activeDrops[dropId].object)
	
	QBCore.Functions.Progressbar("drilling_", "Drilling...", math.random(20000, 30000), false, true, {}, {}, {}, {}, function()
		TriggerServerEvent('genz-gunheist:pickUpDrop', dropId)
		inAnimation = false
	end)
end) 
...
RegisterNetEvent('genz-gunheist:useChip')

AddEventHandler('genz-gunheist:useChip', function()
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)

	if dstcheck < 5 then
		if StartHacking() then
			TriggerServerEvent('genz-gunheist:wonHacking')
		end
	end
end)
...
RegisterNetEvent('genz-gunheist:doDecryptionAnimation')
AddEventHandler('genz-gunheist:doDecryptionAnimation', function()
	local ped = PlayerPedId()
	RequestAnimDict('missheist_jewel@hacking')
	TaskPlayAnim(ped, "missheist_jewel@hacking", "hack_loop", 8.0, 1.0, -1, 9, 0, 0, 0, 0)
end)

RegisterNetEvent('genz-gunheist:useDecryptionKey')
AddEventHandler('genz-gunheist:useDecryptionKey', function(item)
	local dstcheck = #(GetEntityCoords(PlayerPedId()) - hackingPosition)
	if dstcheck < 5 then
		TriggerEvent('genz-gunheist:doDecryptionAnimation')
		...
		QBCore.Functions.Progressbar("drilling_", "Decrypting with key...", math.random(10000, 15000), false, true, {}, {}, {}, {}, function()
			...
			TriggerServerEvent('genz-gunheist:server:useDecryptionKey', item)
		end)
	end
end)
...
function hackingCompleted(success, timeremaining)
	if success then	
		TriggerServerEvent('genz-gunheist:hackingCompleted')
	end

	TriggerEvent('mhacking:hide')
end

RegisterNetEvent('genz-gunheist:hackingMinigame')
AddEventHandler('genz-gunheist:hackingMinigame', function()
