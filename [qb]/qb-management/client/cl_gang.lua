...
RegisterNetEvent('qb-gangmenu:client:OpenMenu', function()
    ...
    local gangMenu = {
        {
            header = LangManagement:t("headersgang.bsm").. string.upper(PlayerGang.label),
            icon = "fa-solid fa-circle-info",
            isMenuHeader = true,
        },
        {
            header = LangManagement:t("bodygang.manage"),
            txt = LangManagement:t("bodygang.managed"),
        ...
        {
            header = LangManagement:t("bodygang.hire"),
            txt = LangManagement:t("bodygang.hired"),
        ...
        {
            header = LangManagement:t("bodygang.storage"),
            txt = LangManagement:t("bodygang.storaged"),
        ...
        {
            header = LangManagement:t("bodygang.outfits"),
            txt = LangManagement:t("bodygang.outfitsd"),
        ...
        -- {
        --     header = LangManagement:t("bodygang.money"),
        --     txt = LangManagement:t("bodygang.moneyd"),
    ...
    gangMenu[#gangMenu + 1] = {
        header = LangManagement:t("bodygang.exit"),
...
RegisterNetEvent('qb-gangmenu:client:ManageGang', function()
    local GangMembersMenu = {
        {
            header = LangManagement:t("bodygang.mempl").. string.upper(PlayerGang.label),
            icon = "fa-solid fa-circle-info",
            isMenuHeader = true,
        },
    }
    QBCore.Functions.TriggerCallback('qb-gangmenu:server:GetEmployees', function(cb)
        ...
        GangMembersMenu[#GangMembersMenu + 1] = {
            header = LangManagement:t("bodygang.return"),
            icon = "fa-solid fa-angle-left",
            params = {
...
RegisterNetEvent('qb-gangmenu:lient:ManageMember', function(data)
    local MemberMenu = {
        {
            header = LangManagement:t("bodygang.mngpl").. data.player.name .. " - " .. string.upper(PlayerGang.label),
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info",
        },
    }
    for k, v in pairs(QBCore.Shared.Gangs[data.work.name].grades) do
        MemberMenu[#MemberMenu + 1] = {
            header = v.name,
            txt = LangManagement:t("bodygang.grade") .. k,
            params = {
                isServer = true,
    ...
    MemberMenu[#MemberMenu + 1] = {
        header = LangManagement:t("bodygang.fireemp"),
...
RegisterNetEvent('qb-gangmenu:client:HireMembers', function()
    local HireMembersMenu = {
        {
            header = LangManagement:t("bodygang.hireemp") .. string.upper(PlayerGang.label),
            isMenuHeader = true,
            icon = "fa-solid fa-circle-info",
        },
    }
        ...
        for _, v in pairs(players) do
            if v and v ~= PlayerId() then
                HireMembersMenu[#HireMembersMenu + 1] = {
                    header = v.name,
                    txt = LangManagement:t("bodygang.cid") .. v.citizenid .. " - ID: " .. v.sourceplayer,
...
RegisterNetEvent('qb-gangmenu:client:SocietyMenu', function()
    QBCore.Functions.TriggerCallback('qb-gangmenu:server:GetAccount', function(cb)
        local SocietyMenu = {
            {
                header =  LangManagement:t("bodygang.balance").. comma_valueGang(cb) .. " - " .. string.upper(PlayerGang.label),
                isMenuHeader = true,
                icon = "fa-solid fa-circle-info",
            },
            {
                header = LangManagement:t("bodygang.deposit"),
                icon = "fa-solid fa-money-bill-transfer",
                txt = LangManagement:t("bodygang.depositd"),
                params = {
            ...
            {
                header = LangManagement:t("bodygang.withdraw"),
                icon = "fa-solid fa-money-bill-transfer",
                txt = LangManagement:t("bodygang.withdrawd"),
                params = {
...
RegisterNetEvent('qb-gangmenu:client:SocietyDeposit', function(saldoattuale)
    local deposit = exports['qb-input']:ShowInput({
        header = LangManagement:t("bodygang.depositm").. saldoattuale,
        submitText = LangManagement:t("bodygang.submit"),
        inputs = {
            {
                type = 'number',
                isRequired = true,
                name = 'amount',
                text = LangManagement:t("bodygang.amount")
            }
        }
    })
    if deposit then
...
RegisterNetEvent('qb-gangmenu:client:SocietyWithdraw', function(saldoattuale)
    local withdraw = exports['qb-input']:ShowInput({
        header = LangManagement:t("bodygang.withdrawm").. saldoattuale,
        submitText = LangManagement:t("bodygang.submit"),
        inputs = {
            {
                type = 'number',
...
CreateThread(function()
    if ConfigManagement.UseTarget then
        for gang, zones in pairs(ConfigManagement.GangMenuZones) do
                    ...
                    options = {
                        {
                            ...
                            label = LangManagement:t("targetgang.label"),
        ...
        while true do
            ...
            if PlayerGang then
                wait = 0
                for k, menus in pairs(ConfigManagement.GangMenus) do
                            ...
                            if #(pos - coords) < 5.0 then
                                ...
                                if #(pos - coords) <= 1.5 then
                                    ...
                                    if not shownGangMenu then
                                        exports['qb-core']:DrawText(LangManagement:t("drawtextgang.label"), 'left')
                                    end
