...
lib.callback.register('oxa-adminmenu:callback:GetPlayers', function(source)
    return getPlayers()
end)

-- Set Job
RegisterNetEvent('oxa-adminmenu:server:SetJob', function(data, selectedData)
...
-- Set Gang
RegisterNetEvent('oxa-adminmenu:server:SetGang', function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local src = source
...
-- Set Perms
RegisterNetEvent("oxa-adminmenu:server:SetPerms", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local src = source
...
-- Remove Stress
RegisterNetEvent("oxa-adminmenu:server:RemoveStress", function(data, selectedData)
    local data = CheckDataFromKey(data)
    if not data or not CheckPerms(source, data.perms) then return end
    local src = source
    ...
    TriggerClientEvent('oxa-adminmenu:client:removeStress', targetId)
