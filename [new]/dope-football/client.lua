...
-- Create blip when the script starts
Citizen.CreateThread(function()
    -- Coordinates for the blip (you can change these)
    local x, y, z = 761.55, -252.04, 64.53

    -- Create the blip
    local blip = AddBlipForCoord(x, y, z)

    -- Set the blip properties (you can customize these)
    SetBlipSprite(blip, 309)  -- Blip icon (1 is a standard point of interest icon)
    SetBlipDisplay(blip, 4)  -- Display type (4 is visible on the map)
    SetBlipScale(blip, 0.9)  -- Size of the blip
    SetBlipColour(blip, 2)   -- Blip color (2 is green, you can change this)
    SetBlipAsShortRange(blip, true)  -- Make the blip only visible when near it

    -- Add a label to the blip (optional)
    BeginTextCommandSetBlipName("STRING")
    AddTextComponentString("Foot Ball")
