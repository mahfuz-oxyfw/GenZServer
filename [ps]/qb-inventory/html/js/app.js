...
$(document).on("click", ".weapon-attachments-back", function (e) {
    e.preventDefault();
    $("#mrf-inventory").css({ display: "block" });
...
$(document).on("click", "#weapon-attachments", function (e) {
    ...
    if (!Inventory.IsWeaponBlocked(ClickedItemData.name)) {
        ...
        $("#mrf-inventory").css({ display: "none" });
...
function swap($fromSlot, $toSlot, $fromInv, $toInv, $toAmount) {
    ...
    var fromData = $fromInv.find("[data-slot=" + $fromSlot + "]").data("item");
    var toData = $toInv.find("[data-slot=" + $toSlot + "]").data("item");

    // Prevent swapping if destination slot contains a different item
    if (toData !== undefined && toData !== null && toData.name !== fromData.name) {
        InventoryError($fromInv, $fromSlot);  // Trigger error for different item
        $.post(`https://${GetParentResourceName()}/Notify`, JSON.stringify({
            message: "You cannot swap different items.",
            type: "error",
        }));
        return; // Stop further execution if trying to swap different items
    }
