...
function TakeOutImpound(vehicle)
    local coords = vector4(458.4957, -992.6875, 25.3503, 178.6785)
    if coords then
...
local workbenchCoords = Config.Locations["impound"][1]
local workbenchHeading = 267.7067
local workbenchModel = `prop_parkingpay` --imp_prop_impexp_mechbench old prop

CreateThread(function()
    RequestModel(workbenchModel)
    while not HasModelLoaded(workbenchModel) do
        Wait(0)
    end

    local workbench = CreateObject(workbenchModel, workbenchCoords.x, workbenchCoords.y, workbenchCoords.z, true, false, false)
    SetEntityHeading(workbench, workbenchHeading)
    FreezeEntityPosition(workbench, true)
    SetModelAsNoLongerNeeded(workbenchModel)
end)



CreateThread(function()
    local coords = Config.Locations["impound"][1]

    exports["qb-target"]:AddCircleZone("impound_zone", vector3(coords.x, coords.y, coords.z + 0.5), 1.5, {
        name = "impound_zone",
        debugPoly = false,
        useZ = true,
    }, {
...
CreateThread(function()
    ...
    local evidenceZones = Config.Locations["evidence"]

    for k, coords in pairs(evidenceZones) do
        exports["qb-target"]:AddCircleZone("evidence_zone_" .. k, vector3(coords.x, coords.y, coords.z), 3.0, {
            name = "evidence_zone_" .. k,
            debugPoly = false,
            useZ = true,
        }, {
            options = {
