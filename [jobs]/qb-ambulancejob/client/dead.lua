...
RegisterNetEvent('ambu:isDead', function(isDead)
		...
		if isDead or InLaststand then
            ...
            if isDead then
                if not isInHospitalBed then
                    if not deathTime then deathTime = 1 end
                    if deathTime > 0 then
            ...
            elseif InLaststand then
                ...
                if LaststandTime > Config.MinimumRevive then
                    DrawTxt(0.94, 1.38, 1.1, 1.0, 0.6, Lang:t('info.bleed_out', {time = math.ceil(LaststandTime)}), 255, 255, 255, 255)
                else
                    DrawTxt(0.845, 1.35, 1.0, 1.0, 0.6, Lang:t('info.bleed_out_help', {time = math.ceil(LaststandTime)}), 255, 255, 255, 255)
                    if not emsNotified then
                        DrawTxt(0.91, 1.32, 1.0, 1.0, 0.6, Lang:t('info.request_help'), 255, 255, 255, 255)
                    else
                        DrawTxt(0.90, 1.33, 1.0, 1.0, 0.6, Lang:t('info.help_requested'), 255, 255, 255, 255)
                    end
