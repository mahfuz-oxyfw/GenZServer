...
--aircraft heists qb-heist
local function AircraftCarrierHeistAlert()
    local coords = GetEntityCoords(cache.ped)

    local dispatchData = {
        message = "Aircraft Carrier Heist Initiated", -- Message to show
        codeName = "carrierheist", -- Codename for the alert
        code = "10-90",
        icon = "fas fa-ship", -- Custom icon
        priority = 3, -- Higher priority for serious events
        coords = coords,
        street = GetStreetAndZone(coords),
        heading = GetEntityHeading(cache.ped),
        jobs = { "leo" } -- Specify this is for law enforcement
    }

    TriggerServerEvent('ps-dispatch:server:notify', dispatchData)
end
exports('AircraftCarrierHeistAlert', AircraftCarrierHeistAlert)
...
local function PhoneCall(message, anonymous, job, type)
    ...
    if IsCallAllowed(message) then
        ...
        local dispatchData = {
            ...
            priority = 2,
            coords = coords,
            name = anonymous and '(ID: ' .. GetPlayerServerId(PlayerId()) .. ')' .. locale('anon') or '(ID: ' .. GetPlayerServerId(PlayerId()) .. ') ' .. (PlayerData.charinfo.firstname .. " " .. PlayerData.charinfo.lastname),
