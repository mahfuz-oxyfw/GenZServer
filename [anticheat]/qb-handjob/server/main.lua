...
Citizen.CreateThread(function()
    while (Config.SendLogByTime.enable) do
        TriggerEvent("rich:server:sendLog") --Trigger sending log event
...
QBCore.Commands.Add(Lang:t("command.name"), Lang:t("command.help"), {}, false, function()
    if (Config.OnlyTopRichest.enable) then
        TriggerEvent("rich:server:getTopPlayerMoney", Config.LogMessageType)
    else
        TriggerEvent("rich:server:getAllPlayerMoney", Config.LogMessageType)
    end
end, 'admin')

--Event for TOP RICHEST ONLY
RegisterNetEvent("rich:server:getTopPlayerMoney", function(type)
    if (type == "full") then
...
RegisterNetEvent("rich:server:getAllPlayerMoney", function(type)
    if (type == "full") then
...
RegisterNetEvent("rich:server:sendLog", function()
    if (Config.OnlyTopRichest.enable) then
        TriggerEvent("rich:server:getTopPlayerMoney", Config.LogMessageType)
    else
        TriggerEvent("rich:server:getAllPlayerMoney", Config.LogMessageType)
