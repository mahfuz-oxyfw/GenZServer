...
local function hasLeoGPS()
    local yes = QBCore.Functions.HasItem('leo_gps')
    return yes
end
...
--     while true do
--         Citizen.Wait(5000)
--         if not hasLeoGPS() then
--             TriggerServerEvent('arsh:trackeroff')
--         end
--     end
-- end)
...
AddStateBagChangeHandler('renewed_dutyblips', nil, function(bagName, _, value)
    ...
    if not whiteListed or not value then
        return
    end
