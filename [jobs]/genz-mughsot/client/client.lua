...
CreateThread(function()
    exports[Config.Target]:AddBoxZone('Mugshottarget', vector3(Config.TargetLoc.x, Config.TargetLoc.y, Config.TargetLoc.z), 0.1, 0.3, {
        ...
        options = {
            {  
                event = "genz-mughsot:Client:mugshotinput",
...
RegisterNetEvent("genz-mughsot:Client:mugshotinput", function()
    ...
    TriggerServerEvent("genz-mughsot:server:takemugshot", picture.citizenid)
end)

RegisterNetEvent("genz-mughsot:client:takemugshot", function(officer)
    ...
    Wait(1000)
	QBCore.Functions.TriggerCallback('genz-mughsot:server:GetWebhook', function(Hook)
        if Hook then
            exports['screenshot-basic']:requestScreenshotUpload(tostring(Hook), 'files[]', {encoding = 'jpg'}, function(data)
				...
				TriggerServerEvent('genz-mughsot:server:MugLog', officer, imageURL)
            end)
        end
    end)
    Wait(5000)
