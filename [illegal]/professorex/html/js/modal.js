...
function handleUserChoice(choice) {
    $('#modal').fadeOut();
    $.post('https://professorex/modalChoice', JSON.stringify(choice));
}
...
function showDeleteConfirmationModal() {
    ...
    $('#modal-button-accept').off('click').on('click', function() {
        if (currentDisplayedConversationId !== undefined) {
            $.post('https://professorex/playerDeletedConversation', JSON.stringify(currentDisplayedConversationId));
